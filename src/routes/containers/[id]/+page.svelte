<script lang="ts">
  ...
  }  async function migrateRootfs() {
    const target = migrateTarget.trim();
    migrateError = null;
    if (!target) {
      migrateError = "Enter a target dataset, e.g. slowpool/lxc/ct1.";
      return;
    }
    if (!confirm(`Move the rootfs to ${target}? The container must stay stopped; the old dataset is destroyed after the copy.`))
      return;
    migrating = true;
    try {
      const op = await client().migrateContainerRootfs(id, { target_dataset: target });
    ...
    <div class="grid">
      ...
      <div class="card">
        ...
        <details class="migrate-box">
          <summary class="muted small">Move rootfs to another dataset or pool</summary>
          <label class="field">
            <span>Target dataset</span>
            <input
              bind:value={migrateTarget}
              autocomplete="off"
              placeholder="slowpool/lxc/ct1"
            />
          </label>
          <button
            class="primary"
            disabled={migrating || ct.state === "running"}
            onclick={migrateRootfs}
          >
            {migrating ? "Migrating…" : "Migrate rootfs"}
          </button>
          {#if ct.state === "running"}
            <p class="muted small">Stop the container to migrate its rootfs.</p>
          {/if}
